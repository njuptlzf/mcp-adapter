...
- **D-03:** Pi only uses Branch A (native extension). Branch C (universal MCP stdio) is only for non-Pi MCP-compatible agents. Pi is not offered Branch C as an option in SKILL.md.
...
- **D-08:** Advanced features are best-effort. TUI panel, custom renderers, `/mcp setup` wizard, `/mcp-auth` command remain Pi-only (Branch A). The MCP stdio server (Branch C) implements what the MCP protocol supports: Sampling forwarding, Elicitation forwarding. Everything else gracefully degrades.
