...
5. Run E2E tests via the detected Path (A / B / C)
6. Generate reports under `tests/reports/`
...
`tests/reports/mcp-adapter-test-report.md` (auto-generated by the matrix reporter — see `tests/reporters/matrix-reporter.ts`) contains: overall pass/fail summary; per-phase detail (counts, duration, failures); Capability Gate result (Path A/B/C per adapter); pass criteria checklist; bugs/notes.
